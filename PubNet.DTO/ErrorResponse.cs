...
public class ErrorResponse
{
    ...
    public static ErrorResponse VersionAlreadyExists(string packageName, string packageVersion) =>
        new(new("version-already-exists", $"Version {packageVersion} of {packageName} already exists"));

    public static ErrorResponse VersionOlderThanLatest(string latestPackageVersion) =>
        new(new("version-older-than-latest", $"The version you are trying to upload is older than the latest version ({latestPackageVersion})"));
    ...
    public static ErrorResponse InvalidPubspec(string message) =>
        new(new("invalid-pubspec", $"An error occurred while parsing the pubspec.yaml: {message}"));
